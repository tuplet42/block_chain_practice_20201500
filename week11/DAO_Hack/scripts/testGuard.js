...
async function main() {
  const DAO = await hre.ethers.getContractFactory("SimpleDAO_Guard");
  const dao = await DAO.deploy();
  await dao.waitForDeployment();

  console.log("Guard DAO deployed to:", await dao.getAddress());

  await dao.deposit({
    value: hre.ethers.parseEther("10")
  });

  console.log("Guard DAO funded with 10 ETH");
